                ...
                <div class="flex items-start">
                    ...
                    <div class="ml-10 flex flex-col w-80">
                        <template v-if="editale">
                            ...
                            <textarea placeholder="状态"
                                class="text-sm text-gray-500 outline-none rounded p-1 my-1 border-gray-700"
                                v-model="store.userInfo.user.status"></textarea>

                            <div class="mt-4 text-sm text-gray-500">
                                <div class="flex">
                                    <Bswitch v-model="store.userInfo.user.allow_search"></Bswitch>
                                    <div class="pl-2">{{ store.userInfo.user.allow_search ? '允许' : '不允许' }}被搜索到</div>
                                </div>
                                <div class="mt-2 flex">
                                    <Bswitch v-model="store.userInfo.user.allow_anon_session"></Bswitch>
                                    <div class="pl-2">{{ store.userInfo.user.allow_anon_session ? '允许' : '不允许' }}匿名会话
                                    </div>
                                </div>
                            ...
                            <div class="flex mt-4">
                                <div class="rounded text-sm border-blue-500 cursor-pointer border bg-blue-500 text-white px-2"
                                    @click="saveUserInfo">
                                    保存
                                </div>
                                <div class="rounded text-sm text-blue-500 cursor-pointer border border-blue-500 px-2 mx-1"
                                    @click="() => (editale = false)">
                                    取消
                                </div>
                            </div>
                        ...
                        <template v-if="!editale">
                            ...
                            <div class="text-xs text-gray-400 outline-none rounded py-1"
                                @click="() => (editale = true)">
                                {{ store.userInfo.user.status }}
                            </div>
                            <div class="flex">
                                <div class="text-sm text-blue-500 cursor-pointer" @click="() => (editale = true)">
                                    编辑
                                </div>
                            </div>
                        </template>
