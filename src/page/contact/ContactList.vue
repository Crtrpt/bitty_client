<template>
  <div class="w-80 bg-gray-100 border-r">
    ...
    >
      <div
        class="cell p-2 flex flex-grow cursor-pointer justify-center items-center"
      >
        <img
          class="rounded-full w-8 h-8 flex-grow-0 flex-shrink-0"
          :src="i.user?.avatar || ''"
          @click="
            () => {
...
export default {
  ...
  mounted() {
    ...
    var _this = this;
    api
      .get("contact/list", { user_id: this.store.userInfo.user.user_id })
      .then((res) => {
        if (res.code == 0) {
